...
func _on_body_entered(body: Node2D) -> void:
	if secuencia_en_curso or (una_sola_vez and ya_interactuado) or ControladorJuego.indice_minijuego_actual >= 1 or ControladorJuego.dia_actual > 1 or ControladorJuego.fin_de_jornada == true:
		queue_free()
